...
function createOverlayWindow() {
  readOpenSetting().then((res) => {
    //opensetting value
    const { hideSec, width, height } = res
    //overlay window default size
    overlayWindowProperty.width = width
    overlayWindowProperty.height = height
    //overlay timeout
    HIDE_TIME = hideSec
...
app.whenReady().then(() => {
  ...
  createWindow().then(async (win) => {
    ...
    ipcMain.handle('getOverlayWindowSize', async () => {
      console.log('getOverlayWindowSize' + JSON.stringify(overlayWindowProperty))
    ...
    ipcMain.on('setOverlayHideSec', (e, m) => {
      const sec = m
      console.log('setOverlayHideSec' + sec)
      HIDE_TIME = sec
      updateOpenSetting({ hideSec: sec })
    })
    ...
    ipcMain.handle('delete-img', async (e, m) => {
      console.log(m)

      const path = m
      const output = { success: '', errorMsg: '' }
      ...
      const result = await dialog.showMessageBox(win, {
        ...
        detail: 'This is a confirmation dialog'
      })
